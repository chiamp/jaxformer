...
- `layers.py`: contains the layers of the transformer (implemented as functions to be called for inference)
...
- `requirements.txt`: install by running `pip install -r requirements.txt`. (NOTE: there are some unnecessary dependencies here, but I just copied my virtual environment's dependencies over by running `pip freeze`)