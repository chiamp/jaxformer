...
from inference import make_teacher_force_forward_fn


def make_compute_loss_fn(config: Config) -> Callable[[dict[str, jax.Array], jax.Array, jax.Array], jax.Array]:
  teacher_force_forward = make_teacher_force_forward_fn(config)

  def compute_loss(params: dict[str, jax.Array], input_sequence_batch: jax.Array, target_sequence_batch: jax.Array) -> jax.Array:
...
def split_data(input_data: jax.Array, target_data: jax.Array, train_split_ratio: float, key: jax.Array) -> tuple[jax.Array, jax.Array, jax.Array, jax.Array]:
  ...
  return (
    ...
    target_data[shuffled_indices[:split_index], :],
    input_data[shuffled_indices[split_index:], :],
    target_data[shuffled_indices[split_index:], :],
  )


def sample_batch(input_data: jax.Array, target_data: jax.Array, batch_size: int, key: jax.Array) -> tuple[jax.Array, jax.Array, jax.Array]:
...
def train(config: Config):
  ...
  # Make loss function
  compute_loss = make_compute_loss_fn(config)
